    ...
    try:
        ...
        for dirpath, dirnames, filenames in os.walk(originalPath, topdown=False): # topdown has to be false
            ...
            for direc in dirnames:
                ...
                changeDirName(dirpath.replace("\\", "/"), direc)
    except IOError:
        #print("ERROR: Can't seem to find the given file.")
        messagebox.showerror("Error!", "Please select another folder!")
...
def changeDirName(fullPath: str, folderName: str) -> None:
    ...
    try:
        ...
        copy_tree(pathWithFolderName, newPathWithDirName) # copy everything inside of original path
    except distutils.errors.DistutilsFileError or FileNotFoundError:
        pass
